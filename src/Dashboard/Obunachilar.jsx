...
function Obunachilar() {
  const [data, setData] = useState([]);

  useEffect(() => {
    axios
      .get("https://a510c4f98367eca1.mokky.dev/users")
      .then((res) => {
        setData(res.data);
      })
      .catch((e) => {
        ...
        <table className="w-full mt-0">
          <thead className="w-full sticky top-1 bg-[#474668] z-10">
            <tr>
              ...
              <th className="px-4 py-2">Obuna bo‘lgan vaqti</th>
              <th className="px-4 py-2">Obunaning tugash sanasi</th>
              <th className="px-4 py-2">Tariflar</th>
          ...
          <tbody>
            {data.map((item, index) => (
              ...
              >
                ...
                <td className="px-4 py-2 text-center">{item.date || "N/A"}</td>
                <td className="px-4 py-2 text-center">{item.date || "N/A"}</td>
                <td className="px-4 py-2 text-center">
                  {item.tariflar || "N/A"}
                </td>
